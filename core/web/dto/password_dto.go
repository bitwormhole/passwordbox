...
type Password struct {
	...
	// dto.Password <==> entity.PasswordRef ( 这两个结构是相对应的 )

	ID dxo.PasswordID `json:"id"`

	Head dxo.PasswordBlockID `json:"head"`

	Base

	// pass-path: email/domain/username/scene/

	Path dxo.UniformPath `json:"path"` // 唯一路径

	Email    dxo.EmailAddress `json:"email"`    // 用户的邮箱地址
	Domain   dxo.DomainName   `json:"domain"`   // 被托管的目标域名
	...
	Salt    lang.Hex `json:"salt"`    // 盐 （用于生成密码）
